...
app.controller("latte", ["$scope", "$http", function ($scope, $http) {
        ...
        var getMail = function () {
            ...
            if ($scope.mailMoney - 0) {
                mail.items.push({ itemId: "D130102", itemNum: $scope.mailMoney - 0 });
            }
            if ($scope.mailGold - 0) {
                mail.items.push({ itemId: "D130103", itemNum: $scope.mailGold - 0 });
            }
            if ($scope.mailItemId) {
                mail.items.push({ itemId: $scope.mailItemId, itemNum: $scope.mailItemNum - 0 || 1 });
            }
            return mail;
        };
